...
DECLARE @schema VARCHAR(128)
DECLARE @SQL VARCHAR(254)
SELECT @name = (SELECT TOP 1 [TABLE_NAME] FROM INFORMATION_SCHEMA.TABLES WHERE table_catalog=DB_NAME() AND TABLE_TYPE = 'VIEW' AND TABLE_SCHEMA <> 'sys' ORDER BY [TABLE_NAME])
SELECT @schema = (SELECT TOP 1 [TABLE_SCHEMA] FROM INFORMATION_SCHEMA.TABLES WHERE table_catalog=DB_NAME() AND TABLE_TYPE = 'VIEW' AND TABLE_SCHEMA <> 'sys' ORDER BY [TABLE_NAME])
